#!/usr/bin/env python3
"""
storeshots_ipad.py -- 13-inch iPad App Store screenshots (2048x2732).

App Store Connect requires at least one 13" iPad screenshot when the app
declares iPad support. This composes the same funded phone captures onto the
website-dark brand background at iPad dimensions, so the iPad set matches the
phone set (tools/demo/storeshots.py) visually.

2048x2732 is the classic 12.9"/13" iPad Pro portrait size; App Store Connect
accepts it for the "13-inch iPad" slot (2064x2752 is also accepted).

Usage:
  python3 tools/demo/storeshots_ipad.py \
      --srcdir ~/Build/hash-wallet/demo/screenshots-funded \
      --outdir ~/Build/hash-wallet/demo/appstore-ipad
"""
...
# Same copy + accents as the phone set.
SHOTS = [
    ("01-dashboard.png",
...
def gradient_bg() -> Image.Image:
    ...
    cx, cy, r = int(W * 0.5), int(H * 0.55), int(W * 0.7)
    ImageDraw.Draw(glow).ellipse([cx - r, cy - r, cx + r, cy + r], fill=44)
...
def build(srcdir: Path, src: str, headline, subhead: str, top_crop: int, out: Path) -> None:
    ...
    fh = ImageFont.truetype(F_HEAD, 150)
    y = 200
    ...
    fs = ImageFont.truetype(F_SUB, 62)
    w = d.textlength(subhead, font=fs)
    d.text(((W - w) / 2, y + 20), subhead, font=fs, fill=rgb(WEB_GREY))
    ...
    im = rounded(im, 60)
    ImageDraw.Draw(im).rounded_rectangle([1, 1, sw - 2, sh - 2], 60,
                                         outline=rgb(WEB_MINT) + (235,), width=3)

    px_x = (W - sw) // 2
    px_y = 760

    shadow = Image.new("RGBA", (sw + 160, sh + 160), (0, 0, 0, 0))
    ImageDraw.Draw(shadow).rounded_rectangle([80, 80, sw + 80, sh + 80], 90, fill=(0, 0, 0, 150))
    shadow = shadow.filter(ImageFilter.GaussianBlur(60))
    sh_layer = Image.new("RGBA", bg.size, (0, 0, 0, 0))
    sh_layer.paste(shadow, (px_x - 80, px_y - 56), shadow)
    bg = Image.alpha_composite(bg, sh_layer)
    bg.paste(im, (px_x, px_y), im)

    d = ImageDraw.Draw(bg)
    w = d.textlength(FOOTER, font=fs)
    d.text(((W - w) / 2, H - 150), FOOTER, font=fs, fill=rgb(WEB_MINT))
...
def main() -> None:
    ...
    for i, (src, headline, subhead, top_crop) in enumerate(SHOTS, 1):
        if not (srcdir / src).exists():
            ...
            continue
        build(srcdir, src, headline, subhead, top_crop, outdir / f"appstore-ipad-{i:02d}.png")
